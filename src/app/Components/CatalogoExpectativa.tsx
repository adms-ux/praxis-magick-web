...
export default function CatalogoExpectativa() {
  ...
  const handleWishlist = (productName: string) => {
    setWishlistMsg(`Has añadido ${productName} a tu Lista de Deseos. Te avisaremos cuando esté listo.`);
    setTimeout(() => setWishlistMsg(null), 3000);
  };

  const productosFuturos = [
    {
      id: "velon",
      name: "Velón de Poder — El Heraldo",
      desc: "No es el ritual en sí — es la proclamación que lo autoriza. Enciéndelo para declarar tu intención al inicio de cualquier operación.",
      image: "/placeholder-velon.png"
    },
    {
      id: "polvo",
      name: "Polvo Mágico — El Alquimista",
      desc: "El catalizador que transforma un objeto común en consagrado. Úsalo para vestir velas o marcar el espacio.",
      image: "/placeholder-polvo.png"
    },
    {
      id: "poppet",
      name: "Poppet — Hamelin's Children",
      desc: "Vehículo de magia simpática que permite dirigir la energía de un ritual sin necesidad de contacto directo.",
      image: "/placeholder-poppet.png"
    },
    {
      id: "idolos",
      name: "Ídolos Consagrados",
      desc: "Cada Ídolo (Claunech, Frimost, Clisthert...) se solicita, se talla y se bautiza de forma individual.",
      image: "/placeholder-idolos.png"
    }
  ];
  ...
  return (
    <section className="w-full max-w-6xl mx-auto px-4 md:px-6 py-16 text-gray-200 z-20 relative">
      ...
      <div className="w-full max-w-4xl mx-auto mb-12 bg-black/60 border border-red-900/50 rounded-xl p-6 flex flex-col md:flex-row items-center gap-6 backdrop-blur-sm shadow-[0_0_20px_rgba(127,29,29,0.3)]">
        ...
        <div>
          ...
          <p className="text-xs md:text-sm text-gray-300 font-sans leading-relaxed text-justify">
            Debido a regulaciones aduanales internacionales estrictas sobre extractos de hierbas y botánica, <strong>nuestros productos físicos solo están disponibles para envío dentro de la República Mexicana</strong>. La biblioteca digital y servicios de magia personalizada están abiertos a nivel global.
          </p>
        </div>
      </div>

      {/* ALERTA DE LISTA DE DESEOS */}
      {wishlistMsg && (
      ...
      {/* GRID DE PRODUCTOS (2 columnas móvil, 4 escritorio) */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        {productosFuturos.map((producto) => (
