...
export default function OleumsPage() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Generar llamas más estiradas y numerosas
      for (let i = 0; i < 8; i++) {
        flames.push({
      ...
      for (let i = flames.length - 1; i >= 0; i--) {
        ...
        gradient.addColorStop(0.3, `rgba(${Math.round(currentColor.r)}, ${Math.round(currentColor.g)}, ${Math.round(currentColor.b)}, ${opacity * 0.6})`);
        gradient.addColorStop(1, "rgba(0, 0, 0, 0)");

        ctx.fillStyle = gradient;
        ctx.beginPath();
        // Uso de ellipse para crear lenguas de fuego verticales en lugar de círculos perfectos
  ...
  return (
    <main className="relative min-h-screen bg-black overflow-x-hidden flex flex-col items-center text-white selection:bg-purple-900 selection:text-green-300">
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-6 w-full max-w-5xl pt-4 pb-16">
          ...
          <div className="relative w-full h-full flex items-center justify-center">
            {OLEUMS_DATA.map((oleum, idx) => {
              ...
              return (
                ...
                >
                  {/* Contenedor libre sin border ni blur de fondo para evitar recortes */}
                  <div className="w-48 h-72 md:w-56 md:h-80 relative flex items-center justify-center transition-all duration-700">
                    <Image 
                      src={oleum.image} 
                      alt={oleum.name} 
                      width={220} 
                      ...
                      style={{ 
                        filter: isActive ? `drop-shadow(0 0 35px ${oleum.accentColor}88) drop-shadow(0 15px 25px rgba(0,0,0,0.9))` : 'drop-shadow(0 10px 15px rgba(0,0,0,0.8))' 
                      }}
                    />
                  </div>
                </div>
        ...
        <div className="w-full max-w-2xl mt-16 text-center transition-all duration-500 z-20 relative">
          ...
          <div className="flex justify-center mb-6 min-h-[80px]">
            <Image 
              ...
              onError={(e) => { 
                ...
                e.currentTarget.parentElement!.innerHTML = `<h2 class="text-3xl md:text-4xl font-cinzel font-bold drop-shadow-[0_0_15px_${currentOleum.accentColor}]" style="color: ${currentOleum.accentColor}">${currentOleum.name}</h2>`;
              }}
            />
          </div>
